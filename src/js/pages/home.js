...
function renderCurrentTeam() {
    const teamContainer =
        document.querySelector("#current-team-list");

    const teamCount =
        document.querySelector("#team-count");

    const headerTeamCount =
        document.querySelector("#header-team-count");

    const currentTeam = getStorage(
        storageKeys.currentTeam,
        []
    );
...
function addPokemonToTeam(pokemonId) {
    ...
    if (currentTeam.length >= 6) {
        window.alert(
            "Your team already has six Pokémon."
        );

        return;
    }

    const pokemon = loadedPokemon.find(
...
function addEventListeners() {
    const searchInput =
        document.querySelector("#pokemon-search");
    ...
    nextButton?.addEventListener(
        ...
        async () => {
            const totalPages =
                Math.ceil(totalPokemon / pageSize);
    ...
    grid?.addEventListener("click", (event) => {
        const teamButton =
            event.target.closest("[data-team-id]");
        ...
        if (teamButton) {
            addPokemonToTeam(
                Number(teamButton.dataset.teamId)
            );
        }
    ...
    teamContainer?.addEventListener(
        ...
        (event) => {
            ...
            const pokemonId = Number(
                removeButton.dataset.removeTeamId
            );
